...
for i in range(len(AttackPercentage)):
    for j in range(len(frameworkName)):
        axList[i][0].plot(x1, df[str(frameworkName[j])+str(AttackPercentage[i])], lineStyle[j], color = color[j], markerfacecolor=markerFace[j],
...
plt.savefig('./fig4(a-d)_SG.svg',bbox_inches='tight') 
################################################################################
