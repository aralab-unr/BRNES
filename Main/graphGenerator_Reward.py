...
for i in range(len(AttackPercentage)):
	for j in range(len(frameworkName)):
		axList[i][0].plot(x2, df1[str(frameworkName[j])+str(AttackPercentage[i])], lineStyle[j], color = color[j], markerfacecolor=markerFace[j],
...
## Make sure path exists for this one line of code #################################
plt.savefig('./fig4(a-d)_Reward.svg',bbox_inches='tight') 
################################################################################
plt.show()
