...
#             while not any('True' in sl for sl in doneList): # ends when any agent reaches goal
    ...
    s = stepsListFinal
    with open("./SG/"+str(fileName)+"_DARL_Step", "wb") as Sp:   #Pickling
        pickle.dump(s, Sp)

    r = rewards_all_episodesFinal
    with open("./Reward/"+str(fileName)+"_DARL_Reward", "wb") as Rp:   #Pickling
        pickle.dump(r, Rp)

    q = qtableListFinal
    with open("./Qtable/"+str(fileName)+"_DARL_Qtable", "wb") as Qp:   #Pickling
        pickle.dump(q, Qp)
        
    c = diffAvg5
    with open("./Convergence/"+str(fileName)+"_DARL_convergence", "wb") as Cp:   #Pickling
        pickle.dump(c, Cp)
    

    t = total_time
    with open("./TG/"+str(fileName)+"_DARL_Time", "wb") as Tp:   #Pickling
        pickle.dump(t, Tp)   


    with open("./OutputFile/DARL.txt", "a") as myfile:
        myfile.write("FileName: "+str(fileName)+" : DARL, Time taken: "+str(total_time)+"\n | gridWidth: "+str(gridWidth)+" | gridHeight: "+str(gridHeight)+
